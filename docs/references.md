...
`1.0.1.0` 后续收尾保持同一结构：修复失效盘符阻断自动发现、Windows PowerShell 5.1 默认路径解析，复用包身份核对逻辑，让诊断识别待恢复状态。实装发现管理员进程的包 COM 激活与普通资源管理器环境不同，因此诊断要求从非管理员窗口运行；不增加常驻程序或注册 DLL。具体测试记录见 README。
